...
export interface DrizzleAdapterConfig {
  ...
  /** Processed operation table name. Default: 'sync_operations'. */
...
export function drizzleAdapter(db: DbLike, config: DrizzleAdapterConfig): SyncAdapter {
  ...
  const operationTbl = config.operationTable ?? 'sync_operations'
  ...
  function makeAdapter(conn: DbLike): SyncAdapter {
    const adapter: SyncAdapter = {
      ...
      async ensureSyncTables() {
        // Application-table DDL, including sync indexes, belongs in a reviewed
        // migration generated by the CLI. Boot only owns these two internal
        // tables so a rollback never changes an application's schema.
        ...
        await conn.execute(sql.raw(`
          CREATE TABLE IF NOT EXISTS "${operationTbl}" (
            model TEXT NOT NULL,
            op_id TEXT NOT NULL,
            created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
      ...
      async findTombstonesSince(
        ...
        const result = await conn.execute(paramSql(
          `SELECT * FROM "${tombTbl}" WHERE ${conditions.join(' AND ')} ORDER BY hlc ASC, id ASC LIMIT ${lim + 1}`,
          values,
        ))
        const rows = (result.rows ?? []) as Row[]
        const hasMore = rows.length > lim
        const page = hasMore ? rows.slice(0, lim) : rows
        const out: FindTombstonesSinceResult = { tombstones: page.map(toTombstone) }
        if (hasMore && page.length > 0) {
          const last = page[page.length - 1]!
          out.nextCursor = { hlc: String(last.hlc), id: String(last.id) }
        }
        return out
      },
      ...
      async claimOperation({ model, opId }) {
        const result = await conn.execute(
          sql`INSERT INTO ${sql.identifier(operationTbl)} (model, op_id) VALUES (${model}, ${opId})
