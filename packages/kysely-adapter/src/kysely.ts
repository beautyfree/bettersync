/**
 * Kysely SyncAdapter implementation (Postgres).
 *
 * Uses `CompiledQuery.raw()` for parameterized queries with $1-style params.
 * Same SQL as the pg/pglite adapters — identical Postgres dialect.
 */
...
// biome-ignore lint/suspicious/noExplicitAny: user's DB types unknown
export function kyselyAdapter(db: Kysely<any>, opts: KyselyAdapterOptions = {}): SyncAdapter {
  const hlcField = opts.hlcField ?? 'changed'
  let schema: SyncSchema | null = null

  function s(): SyncSchema {
    if (!schema) throw new Error('kyselyAdapter: call ensureSyncTables first')
    return schema
  }

  function tbl(model: string): string {
    const def = s()[model]
    if (!def) throw new Error(`kyselyAdapter: unknown model "${model}"`)
  ...
  function makeAdapter(conn: Kysely<any>, isTransaction = false): SyncAdapter {
    async function q<T = Row>(text: string, params: unknown[] = []): Promise<T[]> {
      const result = await conn.executeQuery<T>(CompiledQuery.raw(text, params))
      return result.rows
    }
    ...
    const adapter: SyncAdapter = {
      ...
      async transaction(fn) {
        ...
        return conn.transaction().execute(async (tx) => {
