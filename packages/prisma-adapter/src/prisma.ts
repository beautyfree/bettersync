/**
 * Prisma SyncAdapter implementation.
 *
 * Uses `$queryRawUnsafe` / `$executeRawUnsafe` for all operations.
 * Postgres SQL: ON CONFLICT ... WHERE, JSONB @>, (col1, col2) > (val1, val2).
 * Same SQL as the pg and pglite adapters — Prisma talks to Postgres.
 *
 * This approach keeps the adapter universal: it works with ANY Prisma schema
 * without needing to know the generated model types at compile time.
 */
...
/**
 * Minimal Prisma Client interface — just the methods we use.
 * Defined here so we don't need to import @prisma/client at runtime.
 * Both PrismaClient and the interactive transaction client implement these.
 */
...
export function prismaAdapter(prisma: PrismaClientLike, opts: PrismaAdapterOptions = {}): SyncAdapter {
  const hlcField = opts.hlcField ?? 'changed'
  let schema: SyncSchema | null = null
  const rootPrisma = prisma // keep reference for transaction support

  function s(): SyncSchema {
    if (!schema) throw new Error('prismaAdapter: call ensureSyncTables first')
    return schema
  }

  function tbl(model: string): string {
    const def = s()[model]
    if (!def) throw new Error(`prismaAdapter: unknown model "${model}"`)
  ...
  function makeAdapter(conn: PrismaClientLike, isTransaction = false): SyncAdapter {
    /**
     * Query helper: runs $queryRawUnsafe and returns rows.
     * Prisma takes variadic params, so we spread the array.
     */
    async function q<T = Row>(sql: string, params: unknown[] = []): Promise<T[]> {
      return conn.$queryRawUnsafe<T>(sql, ...params)
    }

    /**
     * Execute helper: runs $executeRawUnsafe for DDL / statements
     * that don't return rows.
     */
    async function exec(sql: string, params: unknown[] = []): Promise<void> {
      await conn.$executeRawUnsafe(sql, ...params)
    }
    ...
    const adapter: SyncAdapter = {
      ...
      async ensureSyncTables(s_) {
        ...
        for (const [modelKey, def] of Object.entries(s_)) {
          ...
          await exec(`CREATE TABLE IF NOT EXISTS "${table}" (${colDefs.join(', ')})`)
          if (hasHlc(modelKey)) {
            await exec(
      ...
      async delete({ model, where }) {
        ...
        await exec(`DELETE FROM "${table}" WHERE ${w.sql}`, w.params)
      },
      ...
      async upsertTombstoneIfNewer(t) {
        ...
        await exec(`DELETE FROM "${table}" WHERE "${pkName}" = $1`, [t.id])
        return true
      },
      ...
      async transaction(fn) {
        if (isTransaction) {
          // Already in a transaction — Prisma doesn't support nested interactive transactions.
          // Just run inline.
          return fn(adapter)
        }
        return rootPrisma.$transaction(async (tx) => {
          const txAdapter = makeAdapter(tx as PrismaClientLike, true)
  ...
  return columnMappingAdapter(makeAdapter(prisma), hlcField)
}
